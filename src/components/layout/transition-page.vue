...
<script>
  import animates from './animate.config'
  export default {
    ...
    props: {
      ...
      animate: {
        type: String,
        validator(value) {
          return animates.preset.findIndex(item => item.name == value) != -1
        },
        default: 'bounce'
      },
      direction: {
    ...
    methods: {
      activeClass(isLeave) {
        let animate = animates.preset.find(item => this.animate == item.name);
        ...
        console.log(animate.name + t + direction);
